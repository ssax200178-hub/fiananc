...
async function searchPastLogsApp() {
  const sevenDaysAgo = new Date();
  sevenDaysAgo.setDate(sevenDaysAgo.getDate() - 7);
  
  console.log('Searching logs in "app" since ' + sevenDaysAgo.toISOString());
  const snapshot = await db.collection('app').doc('v1_data').collection('activity_logs')
    .where('timestamp', '>=', sevenDaysAgo.toISOString())
    .get();

  console.log(`Found ${snapshot.size} total logs in "app" for the last 7 days.`);
...
searchPastLogsApp().then(() => process.exit(0)).catch(err => {
  console.error(err);
  process.exit(1);
});
